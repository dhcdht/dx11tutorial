...
public:
	...
	bool initialize(ID3D11Device *aD3DDevice, char *aModelFileName, WCHAR *aTextureFileName);
	void shutdown();
...
private:
	...
	bool readFileCounts(char *aModelFileName, 
		int &vertexCount, int &textureCount, 
		int &normalCount, int &faceCount);
	bool loadDataStructures(char *aModelFileName, 
		int aVertexCount, int aTextureCount,
		int aNormalCount, int aFaceCount);
	void releaseModel();
