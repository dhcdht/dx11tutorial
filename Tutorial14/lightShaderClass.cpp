...
bool LightShaderClass::initialize(ID3D11Device* aD3DDevice, HWND aHwnd)
{
	...
	result = initializeShader(
		aD3DDevice, aHwnd, L"./shader/light.vsh", L"./shader/light.psh");
	if(!result)
	{
		return false;
	}
	...
	float aSpecularPower)
{
	...
	result = setShaderParameters(
		...
		aCameraPosition, aSpecularColor, aSpecularPower);
	if(!result)
	{
		return false;
	}
	...
	float aSpecularPower)
{
	...
	dataPtr3->cameraPosition = aCameraPosition;
	...
	aD3DDeviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_cameraBuffer);
	...
	dataPtr2->specularColor = aSpecularColor;
	dataPtr2->specularPower = aSpecularPower;
